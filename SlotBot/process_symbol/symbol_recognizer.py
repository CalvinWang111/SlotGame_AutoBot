...
def get_grid_info(points, tolerance=30):
    debug_img = np.zeros([1080,1920,3],np.uint8)
    for point in points:
        cv2.circle(debug_img,(int(point[0]),int(point[1])),2,(0,255,0), -1)
    ...
    row_number = round((max(row_y)-min(row_y))/avg_row_height+1)
    column_number = round((max(col_x)-min(col_x))/avg_col_width+1)
    
    return (display_x, display_y, display_width, display_height), (row_number,column_number)
