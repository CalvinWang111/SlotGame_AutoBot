...
def convert_symbol_to_template(symbol_path, template_path):
    ...
    _, thresh = cv2.threshold(alpha_channel, 64, 255, cv2.THRESH_BINARY)
    ...
    print(f'Original shape: {alpha_channel.shape}, New shape: {template.shape}')
...
for base_symbol_path in base_symbol_dir.glob('*.png'):
    print(f'Converting {base_symbol_path.name} to template')
